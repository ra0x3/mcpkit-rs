...
async fn test_client_init_handles_jsonrpc_error() {
    ...
    let mut server = IntoTransport::<mcpkit_rs::RoleServer, _, _>::into_transport(server_transport);
    ...
    match result {
        Err(mcpkit_rs::service::ClientInitializeError::JsonRpcError(error_data)) => {
