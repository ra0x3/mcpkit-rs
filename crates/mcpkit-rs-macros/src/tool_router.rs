...
pub fn tool_router(attr: TokenStream, input: TokenStream) -> syn::Result<TokenStream> {
    ...
    // find all function marked with `#[mcpkit_rs::tool]`
    ...
    let router_fn = syn::parse2::<ImplItem>(quote! {
        #vis fn #router() -> mcpkit_rs::handler::server::router::tool::ToolRouter<Self> {
            mcpkit_rs::handler::server::router::tool::ToolRouter::<Self>::new()
                #(#routers)*
        }
    })?;
