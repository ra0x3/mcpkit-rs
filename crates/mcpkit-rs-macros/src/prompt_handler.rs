...
pub fn prompt_handler(attr: TokenStream, input: TokenStream) -> syn::Result<TokenStream> {
    ...
    let get_prompt_impl: ImplItem = parse_quote! {
        ...
        ) -> Result<GetPromptResult, mcpkit_rs::ErrorData> {
            let prompt_context = mcpkit_rs::handler::server::prompt::PromptContext::new(
                self,
                request.name,
                request.arguments,
                context,
    ...
    let list_prompts_impl: ImplItem = parse_quote! {
        ...
        ) -> Result<ListPromptsResult, mcpkit_rs::ErrorData> {
